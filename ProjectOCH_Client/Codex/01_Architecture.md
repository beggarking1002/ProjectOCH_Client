...
- `BattleField_001`은 원본 지형 `Ground_Tilemap`, 정적 이동 불가 요소 `Prop_Tilemap`, 동적 전투 효과 `CombatOverlay_Tilemap`의 3계층이다. Overlay는 Ground를 바꾸지 않으며 Ground보다 위, Prop보다 아래(`sortingOrder` 5)에 렌더링된다.
